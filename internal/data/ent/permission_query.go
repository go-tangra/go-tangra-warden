...
func (_q *PermissionQuery) QueryFolder() *FolderQuery {
...
func (_q *PermissionQuery) QuerySecret() *SecretQuery {
...
func (_q *PermissionQuery) WithFolder(opts ...func(*FolderQuery)) *PermissionQuery {
	query := (&FolderClient{config: _q.config}).Query()
...
func (_q *PermissionQuery) WithSecret(opts ...func(*SecretQuery)) *PermissionQuery {
	query := (&SecretClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PermissionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Permission, error) {
	...
	if _q.withFolder != nil || _q.withSecret != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withFolder; query != nil {
		if err := _q.loadFolder(ctx, query, nodes, nil,
			func(n *Permission, e *Folder) { n.Edges.Folder = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withSecret; query != nil {
		if err := _q.loadSecret(ctx, query, nodes, nil,
			func(n *Permission, e *Secret) { n.Edges.Secret = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *PermissionQuery) loadFolder(ctx context.Context, query *FolderQuery, nodes []*Permission, init func(*Permission), assign func(*Permission, *Folder)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Permission)
	for i := range nodes {
		if nodes[i].folder_permissions == nil {
			continue
		}
		fk := *nodes[i].folder_permissions
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "folder_permissions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *PermissionQuery) loadSecret(ctx context.Context, query *SecretQuery, nodes []*Permission, init func(*Permission), assign func(*Permission, *Secret)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Permission)
	for i := range nodes {
		if nodes[i].secret_permissions == nil {
			continue
		}
		fk := *nodes[i].secret_permissions
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "secret_permissions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
