...
func (_q *SecretQuery) QueryFolder() *FolderQuery {
...
func (_q *SecretQuery) QueryVersions() *SecretVersionQuery {
	query := (&SecretVersionClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *SecretQuery) QueryPermissions() *PermissionQuery {
...
func (_q *SecretQuery) WithFolder(opts ...func(*FolderQuery)) *SecretQuery {
	query := (&FolderClient{config: _q.config}).Query()
...
func (_q *SecretQuery) WithVersions(opts ...func(*SecretVersionQuery)) *SecretQuery {
	query := (&SecretVersionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *SecretQuery) WithPermissions(opts ...func(*PermissionQuery)) *SecretQuery {
	query := (&PermissionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *SecretQuery) prepareQuery(ctx context.Context) error {
	...
	if secret.Policy == nil {
		return errors.New("ent: uninitialized secret.Policy (forgotten import ent/runtime?)")
	}
	if err := secret.Policy.EvalQuery(ctx, _q); err != nil {
		return err
	}
	return nil
}
...
func (_q *SecretQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Secret, error) {
	...
	if query := _q.withFolder; query != nil {
		if err := _q.loadFolder(ctx, query, nodes, nil,
			func(n *Secret, e *Folder) { n.Edges.Folder = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withVersions; query != nil {
		if err := _q.loadVersions(ctx, query, nodes,
			func(n *Secret) { n.Edges.Versions = []*SecretVersion{} },
			func(n *Secret, e *SecretVersion) { n.Edges.Versions = append(n.Edges.Versions, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withPermissions; query != nil {
		if err := _q.loadPermissions(ctx, query, nodes,
			func(n *Secret) { n.Edges.Permissions = []*Permission{} },
			func(n *Secret, e *Permission) { n.Edges.Permissions = append(n.Edges.Permissions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SecretQuery) loadFolder(ctx context.Context, query *FolderQuery, nodes []*Secret, init func(*Secret), assign func(*Secret, *Folder)) error {
...
func (_q *SecretQuery) loadVersions(ctx context.Context, query *SecretVersionQuery, nodes []*Secret, init func(*Secret), assign func(*Secret, *SecretVersion)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(secretversion.FieldSecretID)
	}
	query.Where(predicate.SecretVersion(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(secret.VersionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *SecretQuery) loadPermissions(ctx context.Context, query *PermissionQuery, nodes []*Secret, init func(*Secret), assign func(*Secret, *Permission)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Secret)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Permission(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(secret.PermissionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.secret_permissions
		if fk == nil {
			return fmt.Errorf(`foreign-key "secret_permissions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "secret_permissions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *SecretQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withFolder != nil {
			_spec.Node.AddColumnOnce(secret.FieldFolderID)
		}
	}
