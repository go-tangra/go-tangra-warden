...
test.describe('warden permissions', () => {
  ...
  test('grants viewer to a role on a folder, shows the effective view elsewhere, then revokes', async ({ page, browser }) => {
    ...
    await expect(drawer).toContainText('Your relation: owner')
    await drawer.locator('#perm-subject').fill('Role: operator')
    await page.getByRole('option', { name: /operator/i }).locator('button').click()
    await drawer.locator('#perm-level').selectOption('viewer')
    await drawer.getByRole('button', { name: 'Grant' }).click()
    ...
    await other.getByRole('treeitem', { name: folderName, exact: true }).click()
