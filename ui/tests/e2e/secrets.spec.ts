...
test.describe('warden secrets', () => {
  ...
  test('creates a folder and a secret, reveals, rotates, restores, moves and deletes', async ({ page }) => {
    ...
    await drawer.getByTestId('secret-name').locator('input').fill(secretName)
    await drawer.getByTestId('secret-username').locator('input').fill('root')
    await drawer.getByTestId('secret-host').locator('input').fill('https://db.example.org')
    await drawer.getByTestId('secret-password').locator('input').fill(marker)
    await drawer.getByTestId('secret-totp').locator('input').fill('JBSWY3DPEHPK3PXP')
    ...
    await expect(page.getByTestId('notice')).toContainText('version 4')
    await versions.getByTestId('versions-close').click()
    await expect(drawer).toBeVisible()

    // Move to the root and delete.
    await drawer.getByTestId('secret-folder').click()
